...
class BasicDataModule(LightningDataModule):
    ...
    def _get_tokenizer(self, raw_data):
        # Using a simpler tokenizer loading strategy for now, or adapt construct_tokenizer
        ...
        # tokenizer = construct_tokenizer(raw_data, self.config.data.tok_name, self.config.data.model_max_length, self.config.model.vocab_size, self.config.cache_dir,
        #                                 self.config.data.preprocess_batch_size,
        #                                 special_tokens=self.special_tokens if hasattr(self, "special_tokens") else {})
        # tokenizer.save_pretrained(CONFIG.save_dir)
        return tokenizer

    def _load_tokenizer(self):
        return AutoTokenizer.from_pretrained(self.config.data.tokenizer_name_or_path)
        # return load_tokenizer(
        #    self.config.data.tok_name,
        #    seq_length=self.config.data.model_max_length,
        # )
    ...
    def _log_tokenization(self, train_dataset):
        ...
        # Using print instead of log for simplicity if log is not defined
        print(
            f"Random sentence with seq_length {self.config.data.max_seq_len} from dataset of size {dataset_size:,}: ..."
        )
        print(input_data)
        print("... is tokenized into ...")
        print(self.tokenizer.encode(input_data))
