...
from .qasper_funcs import (
    extract_answer_and_evidence,
    get_evidence_mask,
    get_paragraphs_from_article,
    text_to_instance,
)


def qasper_preprocess(
    examples,
    tokenizer,
    hdt_tokenizer,
    global_query,
    ...
    documents = []
    batch_types = []
    for title, abstract, full_text, qas, id in zip(
        examples["title"],
        examples["abstract"],
        examples["full_text"],
        examples["qas"],
        ...
        for question, answers in zip(qas["question"], qas["answers"]):
            ...
            answers_to_yield = (
                [x["text"] for x in all_answers]
                if for_training
                else [all_answers[0]["text"]]
            )
            types_to_yield = (
                [x["type"] for x in all_answers]
                if for_training
                else [all_answers[0]["type"]]
            )
            evidence_masks_to_yield = (
                all_evidence_masks if for_training else [all_evidence_masks[0]]
            )
            evidence_to_yield = all_evidence if for_training else [all_evidence[0]]
            for answer, evidence, evidence_mask, q_type in zip(
                answers_to_yield,
                evidence_to_yield,
                evidence_masks_to_yield,
                types_to_yield,
            ):
                if context == "question_and_evidence" and answer in [
                    "Unanswerable",
                    "Yes",
                    "No",
                ]:
    ...
    tokenized_data = dict(
        zip(
            outputs[0].keys(),
            [
                np.concatenate([np.expand_dims(d[key], axis=0) for d in outputs])
