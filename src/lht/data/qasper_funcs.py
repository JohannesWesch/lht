...
def get_evidence_mask(evidence: List[str], paragraphs: List[List[str]]) -> List[int]:
    ...
    for paragraph in paragraphs:
        for evidence_str in evidence:
            ...
            if evidence_str in " ".join(paragraph) or evidence_str in "".join(
                paragraph
            ):
...
def text_to_instance(
    ...
    tokenization_outputs = hierarchical_tokenizer(
        paragraphs,
    ...
    if evidence_mask is not None:
        evidence_labels = np.zeros_like(
            tokenization_outputs["input_ids"], dtype=np.int32
        )
        sec_ids = (tokenization_outputs["keep_ids_2"] == 1) & (
            tokenization_outputs["keep_ids_1"] == 1
        )
    ...
    for paragraph in paragraphs:
        ...
        if paragraph_separator:
            tokenized_context.append(
                tokenizer.convert_tokens_to_ids([paragraph_separator])[0]
            )
    if paragraph_separator:
